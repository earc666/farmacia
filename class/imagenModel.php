...
class imagenModel extends Modelo
{
	...
	public function getImagenes(){
		$img = $this->_db->query("SELECT img.nombre as imagen, p.id, p.nombre as producto, p.precio FROM imagenes img INNER JOIN productos p ON img.producto_id = p.id WHERE img.portada = 1 AND p.active = 1");
	...
	public function getImagenUsuario($usuario){
		$usuario = (int) $usuario;

		$img = $this->_db->prepare("SELECT id, titulo, descripcion, nombre FROM imagenesusuario WHERE usuario_id = ?");
		$img->bindParam(1, $usuario);
		$img->execute();

		return $img->fetchall();
	}
	
	public function setImagenusuario($titulo, $descripcion, $imagen, $producto, $portada){
		$producto = (int) $producto;
		$portada = (int) $portada;

		$img = $this->_db->prepare("INSERT INTO imagenesusuario VALUES(null, ?, ?, ?, ?, ?, now(), now())");
		$img->bindParam(1, $titulo);
		$img->bindParam(2, $descripcion);
		$img->bindParam(3, $imagen);
		$img->bindParam(4, $producto);
